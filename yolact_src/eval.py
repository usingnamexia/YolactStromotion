...
class args_xia():
    def __init__(self):
        ...
        self.images = None
        self.video = None
        self.video_multiframe = 1
        self.score_threshold = 0.1
        self.dataset = None
        self.detect = False
        self.display_fps = False
        self.emulate_playback = False
        self.no_bar=False;self.display=False;self.resume=False;self.output_coco_json=False;self.output_web_json=False;self.shuffle=False;self.benchmark=False;self.no_sort=False;self.no_hash=False;self.mask_proto_debug=False;self.crop=True;self.detect=False;self.display_fps=False;self.emulate_playback=False
...
def evalimagexia(net:Yolact, im):
    frame = torch.from_numpy(im).cuda().float()
    batch = FastBaseTransform()(frame.unsqueeze(0))
    preds = net(batch)
    h,w = im.shape[0],im.shape[1]
    masks = prep_benchmarkxia(preds,h,w)
    return masks
    
def prep_benchmarkxia(dets_out, h, w):
    with timer.env('Postprocess'):
        t = postprocess(dets_out, w, h, crop_masks=args.crop, score_threshold=args.score_threshold)
    ...
    with timer.env('Copy'):
        classes, scores, boxes, masks = [x[:args.top_k] for x in t]
        if isinstance(scores, list):
            box_scores = scores[0].cpu().detach().numpy()
            mask_scores = scores[1].cpu().detach().numpy()
        else:
    ...
    for bbox in boxes:
        x1,y1,x2,y2 = bbox
        if classes[i]==0:
            list_H.append(abs(y1-y2))
            list_num.append(i)
        i+=1
    if len(list_H)==0:
        return False
    return masks[list_num[list_H.index(max(list_H))]]
